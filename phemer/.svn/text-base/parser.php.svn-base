...
class PhemeParser extends PhemeBaseParser {
     ...
     * @var string
     */
    protected $_preg = '!{([@$])([A-Za-z0-9][A-Za-z0-9.]+)}|{([A-Za-z][A-Za-z0-9]*:?[A-Za-z0-9]*)(\((.+?)\))?(/}|}(.*?){/\\3})!s';
     ...
     * Class constructor
     *
     * @param array $vars Associative array of blocks, block skins and variables
    ...
    function __construct($rules = array(), $options = array()) {
        ...
        if ($rules) {
            foreach ($rules as $name => $value) {
                ...
                // block skins (no default value)
                    $this->blocks[$value] = new PhemeSkinParser();
                }
                elseif ($name{0} == '$') {
                // variable
                    $this->vars[substr($name, 1)] = $value;
                }
                elseif ($name{0} == '@') {
                // variable
                    $this->skins[substr($name, 1)] = $value;
                }
                elseif (is_string($value)) {
                // block skins
                    $this->skins[$name] = $value;
                    $this->blocks[$name] = new PhemeSkinParser();
                }
        ...
        if (!empty($matches[2])) {
            ...
            if (is_array($params)) {
                foreach ($params as &$param) {
                    if (is_string($param) && strpos($param, '{')) {
                        $param = PhemeParser::parse($param, 'param');
                    }
                }
            }
    ...
    protected function _getSkin($name, $parse = true) {
        ...
        for ($i = count(self::$parseCallStack) - 1; $i >= 0; $i--) {
            if (isset(self::$parseCallStack[$i]->skins[$name])) {
                ...
                if ($parse) {
                    $skin = self::$parseCallStack[$i]->parse($skin, 'skins');
                }
                return $skin;
            }
        }
    ...
    protected function block_pattern($block_name) {
       return '/{block:('.$block_name.')}(.*?){\/block:\\1}/is';
    }
    
    protected function getBlockWithLabel($html, $block_name) {
        
        $matches = array();
        
        $block_pattern = $this->block_pattern($block_name);
        preg_match($block_pattern, $html, $matches);
        if (!empty($matches)) {
            return $matches[2];
        } else {
            return false;
...
class PhemeSubParser extends PhemeParser {
    ...
    function __construct() {
        ...
        parent::__construct(array_shift($this->params), array_shift($this->params));
    }
}
...
class PhemeLoopParser extends PhemeSubParser {
    ...
    public function parse($html = null, $blockName = 'document', $blockParams = null, $noCycle = false) {
        ...
        if (!empty($blockParams['itemTag'])) {
            if (!empty($blockParams['groupTag'])) {
                ...
                foreach ($param as $vars) {
                    ...
                    $items[] = $this->_parseItem($itemTags[$i][$j], $vars, $blockName, $blockParams);
                    $j++;
        ...
        // the simplest cycle. We use $this->_params[0], stored by the class constructor
            ...
            foreach ($param as $vars) {
                ...
                $items[] = ($first ? '' : $sep) .
                    $this->_parseItem($html, $vars, $blockName, $blockParams);
                $first = false;
            }
        }
    ...
    protected function _parseItem($html, $vars, $blockName = 'document', $blockParams = null) {
        ...
        $this->vars = is_array($vars) ? $vars :
            (empty($blockParams['var']) ?
                array('item' => $vars) :
                array($blockParams['var'] => $vars)
            );
...
class PhemeHtmlEntitiesParser extends PhemeParser {
    ...
    public function parse($html, $blockName = 'document', $blockParams = null) {
        $html = parent::parse($html);
        if (isset($blockParams['maxLength']) && mb_strlen($html) > $blockParams['maxLength']) {
        ...
        return htmlentities(
            ...
            !empty($blockParams['doubleEncode'])
        );
    }
}
...
class PhemeVarParser extends PhemeParser {
     ...
     */
    public function parse($html, $blockName = 'document', $blockParams = null) {
        return parent::parse($this->_getVar($html));
    }
}
